...
public class Client {
    ...
    private static final String    GETALL_OUTCOMMAND = "GAL";
    ...
    private static final String     GETALL_INCOMMAND = "HRE";
    ...
    public ServerList getServerData() throws IOException {
        ServerList result = null;
        writeSocket(GETALL_OUTCOMMAND);
        String receivedCommand = readSocket();
        String prefix = getStringElem(receivedCommand, 1);
        if (prefix.equals(GETALL_INCOMMAND)) {
            String left = receivedCommand;
            result = new ServerList();
            int elementCount  = Integer.parseInt(getStringElem(receivedCommand, 2));
            for (int i = 0; i < elementCount; i++) {
                left = left.substring(left.indexOf('\n') + 1);
                int key = Integer.parseInt(getStringElem(left, 1));
                long fileSize = Long.parseLong(getStringElem(left, 2));
                int ipCount = Integer.parseInt(getStringElem(left, 3));
                ServerElement theElem = new ServerElement(key, fileSize);
                for (int j = 0; j < ipCount; j++) {
                    left = left.substring(left.indexOf('\n') + 1);
                    String ip = getStringElem(left, 1);
                    theElem.addIP(ip);
                }
                result.addElement(theElem);
            }
            System.out.println("Get server data successfully.");
        } else {
            System.out.println("Get server data failed.");
        }
        return result;
    }
